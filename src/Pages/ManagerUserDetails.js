  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        } else if (callHistoryData && Array.isArray(callHistoryData.results)) {
          setCallHistory(callHistoryData.results);
        } else if (callHistoryData && typeof callHistoryData === "object") {
          setCallHistory([callHistoryData]); // Wrap single object in an array
        } else {
          setCallHistory([]);
        }
    ...
    return () => {
      setCallHistory([]); // Optional: Reset call history on unmount or dependency change
      setLoading(false);
    };
  }, [id, page]);
