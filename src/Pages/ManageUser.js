...
const ManageUser = () => {
  ...
  const handleSearch = () => {
    ...
    const filtered = users.filter((user) => {
      ...
      const phoneMatch =
        user.mobile_number &&
        normalizePhone(user.mobile_number).includes(
          searchQuery.replace(/\D/g, "")
        ); // Normalize and match phone number
  ...
  const handleToggleBan = async (userId, isBanned) => {
    try {
      ...
      console.error(
        `Error ${isBanned ? "unbanning" : "banning"} user:`,
        err.message
      );
      toast.error(
        `Error ${isBanned ? "unbanning" : "banning"} user: ${err.message}`
      );
    }
  };
  ...
  const handleToggleSuspend = async (userId, isSuspended) => {
    try {
      ...
      console.error(
        `Error ${isSuspended ? "unsuspending" : "suspending"} user:`,
        err.message
      );
      toast.error(
        `Error ${isSuspended ? "unsuspending" : "suspending"} user: ${
            ...
            <Col className="summary-item">
              ...
              <div className="d-flex align-items-center justify-content-center">
                ...
                <span className="text-danger">
                  {userstatstics?.inactive_users ?? 0}
                </span>
              </div>
            </Col>
            <Col className="summary-item">
              ...
              <React.Fragment key={user["User_ID"]}>
                <tr>
                  <td>
                    <span
                      onClick={() =>
                        navigate(`/manageuserdetails/${user.id}/${user.User_ID}`)
                      }
                    >
                      {user.User_ID}
                    </span>
                  ...
                  >
                    {Math.floor(user.Total_Talktime / 60) +
                      " Min ,  " +
                      (user.Total_Talktime % 60) +
                      "  Sec"}
                  </td>
                  <td>
                    <FaEllipsisV
          ...
          <Modal.Body>
            <div className="d-flex flex-column">
              ...
              <Button
                ...
                onClick={() =>
                  handleToggleSuspend(selectedUser.id, selectedUser.Suspend)
                }
              >
                <FaPhoneSlash className="me-2" />{" "}
                {selectedUser.Suspend ? "Unsuspend User" : "Suspend User"}
              </Button>
              ...
              <Button
                ...
                className="text-start"
                onClick={() =>
                  handleToggleBan(selectedUser.id, selectedUser.Ban)
                }
              >
                <FaBan className="me-2" />{" "}
                {selectedUser.Ban ? "Unban User" : "Ban User"}
              </Button>
            </div>
          </Modal.Body>
        </Modal>
