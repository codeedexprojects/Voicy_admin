...
function ManageExecutive() {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      if (response.status === 201) {
        toast.success("Profile request approved successfully!");
        setShowRequestModal(false);
      } else {
        toast.error("Failed to approve the profile request.");
      }
    } catch (error) {
      console.error("Error approving profile request:", error.message);
      toast.error(
  ...
  const handleDecline = async (id) => {
    try {
      ...
      if (response.status === 201) {
        toast.success("Profile request declined successfully!");
        setShowRequestModal(false);
      } else {
        toast.error("Failed to decline the profile request.");
      }
    } catch (error) {
      console.error("Error declining profile request:", error.message);
      toast.error(
        error.response?.data?.message ||
  ...
  useEffect(() => {
    const fetchExecutives = async () => {
      try {
        ...
        setFilteredExecutives(data.executives);
        setLoading(false);
      } catch (err) {
          ...
          <Row>
            ...
            <Col className="summary-item">
              ...
              <div className="d-flex align-items-center justify-content-center">
                ...
                <span className="text-danger">{status.active_executives}</span>
              </div>
            </Col>
            <Col className="summary-item">
              <p>Active</p>
